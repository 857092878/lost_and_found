...
def login(request):
    ...
    if request.method == "GET":
        form = LoginForm()
        return render(request,"login.html",{"form":form,"title":"管理员登陆"})
    ...
    if form.is_valid():
        ...
        request.session["info"] = {'admin_id': admin_object.id, 'admin_name': admin_object.username}#操作的是电脑上的cookies
        request.session.set_expiry(60*60*24*7)#如果登陆成功，重新设置超时时间,7天不用验证码，免登录
